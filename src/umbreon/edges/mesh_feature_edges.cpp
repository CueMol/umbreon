// Shared feature-edge extraction. See mesh_feature_edges.hpp.
//
// extractMeshFeatureEdges is the factored body of the former
// object_space_edges.cpp:emitMeshEdges: the same weld / emap / cluster-normal
// pipeline and the same silhouette / crease / border predicates, changed ONLY in
// the sink (topology-tagged FeatureSeg instead of disconnected RawSeg) and in
// tracking node ids for chaining.
//
// The extraction runs as four phases over a shared ExtractContext:
//   1) weldCorners            positional weld of the de-indexed corners
//   2) buildFacesAndAdjacency welded face indices, normals, edge adjacency
//   3) detectSilhouettes      smooth n.v==0 contour + hard-edge straddle
//   4) detectCreasesAndBorders dihedral creases + open-boundary borders
#include "edges/mesh_feature_edges.hpp"

#include <array>
#include <cmath>
#include <cstddef>
#include <cstdint>
#include <unordered_map>
#include <vector>

#include "edges/mesh_weld.hpp"

namespace umbreon {
namespace {

constexpr float kPi = 3.14159265358979323846f;

// "Toward the viewer" unit vector at P (orthographic: constant -view direction;
// perspective: direction to the eye). Mirrors object_space_edges.cpp.
Vec3 viewerDirAt(const Vec3& P, const Camera& cam) {
  const Vec3 toCam = normalize(cam.direction * -1.0f);
  if (cam.orthographic) return toCam;
  const Vec3 d = cam.position - P;
  const float l = length(d);
  return l > 0.0f ? d * (1.0f / l) : toCam;
}

// Per-corner attribute reads routed through the optional index. With a
// de-indexed mesh cornerVertex(c)==c, so this is the legacy access.
inline const Vec3& cornerPos(const Mesh& mesh, std::size_t c) {
  return mesh.positions[mesh.cornerVertex(c)];
}
inline const Vec3& cornerNrm(const Mesh& mesh, std::size_t c) {
  return mesh.normals[mesh.cornerVertex(c)];
}
inline float cornerAlpha(const Mesh& mesh, std::size_t c) {
  return mesh.colors[mesh.cornerVertex(c)].w;
}

// Edge adjacency entry (welded-position topology): the up-to-two incident
// faces of a welded mesh edge.
struct EAdj {
  int f1 = -1, f2 = -1;
};

// Canonical 64-bit key of the undirected welded edge (a,b).
inline std::uint64_t ekey(int a, int b) {
  const std::uint32_t lo = static_cast<std::uint32_t>(a < b ? a : b);
  const std::uint32_t hi = static_cast<std::uint32_t>(a < b ? b : a);
  return (static_cast<std::uint64_t>(hi) << 32) | lo;
}

// Shared state threaded through the extraction phases (formerly the locals of
// the single monolithic extractMeshFeatureEdges body).
struct ExtractContext {
  std::size_t nCorner = 0;  // de-indexed corner count (3 * nTri)
  std::size_t nTri = 0;
  std::size_t nV = 0;       // welded vertex count
  bool haveNrm = false;     // mesh carries per-vertex normals
  bool haveCol = false;     // mesh carries per-vertex colors
  float hardCos = 0.0f;     // cos(meshHardEdgeDeg)
  int kSelfRings = 0;       // QI self-exclude ring count (0 = {f0,f1} only)

  // Phase 1: positional weld.
  std::vector<Vec3> vpos;        // welded vertex positions
  std::vector<Vec3> vacc;        // per welded vertex: summed corner normals
  std::vector<float> vAlphaSum;  // per welded vertex: summed corner alpha
  std::vector<int> vAlphaCnt;    //   ... and its corner count
  std::vector<int> corner2v;     // corner -> welded vertex id

  // Phase 2: faces, normals, adjacency.
  std::vector<int> fa, fb, fc;   // per face: welded vertex ids
  std::vector<Vec3> fNg;         // per face: geometric normal
  std::vector<Vec3> vN;          // per welded vertex: averaged normal
  std::vector<Vec3> cnrm;        // per corner: hard-edge cluster normal
  std::unordered_map<std::uint64_t, EAdj> emap;  // welded edge -> faces
  std::vector<std::array<int, 3>> faceNbr;       // face -> edge-adjacent faces

  // Synthetic node ids for smooth-contour crossings interior to a face. A
  // crossing on the welded edge (a,b) gets the SAME id from both incident
  // faces, so the contour chains across the shared edge. Ids start at nV.
  std::unordered_map<std::uint64_t, int> crossNode;
  int nextNode = 0;
};

// 1) POSITIONAL WELD of the de-indexed corners into shared vertices
//    (quantized-position hash), accumulating an averaged vertex normal.
//
// WHY THIS EXISTS (not a lossy shortcut -- it RECONSTRUCTS the true topology):
// CueMol exports each ribbon/cartoon surface as a stack of triangle STRIPS, one
// per sweep "winding", and the vertices (and normals) on the shared border
// between adjacent windings are DUPLICATED -- each winding repeats the seam ring
// with its own indices. So the POV mesh2 `face_indices` encode each strip's
// internal adjacency but carry NO connectivity BETWEEN strips: used as-is the
// surface is a soup of disconnected strips (measured on ribbon1: 1060 connected
// components, 54520 boundary/1-face edges, 0 non-manifold). The real surface
// connectivity is present ONLY as coincident vertex positions. Welding by
// quantized position folds those redundant duplicates back together and recovers
// the intended water-tight mesh (ribbon1 after weld: 1 component, ~1144 genuine
// boundary edges = caps/strand-ends). The welded mesh IS the true topology; the
// per-strip index is the redundant representation. (De-indexing upstream is also
// unavoidable: mesh2 carries per-CORNER normals and per-corner texture/color
// indices that differ across a shared position, e.g. at a hard edge.)
//
// WHAT IT DRIVES: the averaged vertex normal (vN), the edge adjacency map (emap,
// for hard-edge silhouette + crease/border), the smooth-contour chain node ids,
// and the QI self-face exclude set.
//
// LIMITATION: the merge is position-only, so it ALSO stitches genuinely-distinct
// sheets that happen to pass within kQuant of each other (e.g. a tight self-fold,
// or many strip ends converging at a cap), producing a few non-manifold edges
// (ribbon1: 72, max 16 faces on one edge). This is harmless to silhouette
// EXTRACTION; it only constrains a Freestyle-style TOPOLOGICAL QI self-skip
// (sharing a vertex no longer implies "same continuous sheet"). A normal/
// orientation-aware weld would separate those touches while still stitching the
// smooth winding seams. NOTE: this weld is unrelated to the smooth-silhouette
// CUSP visibility gap (the missing outline at grazing twists); that is a contour
// fold-back handled by Freestyle's computeCusps, independent of topology.
// The weld key (WeldKey/weldKey, edges/mesh_weld.hpp) and ~1e-4 tolerance are
// shared with the mesh2 reader, which bakes the very same partition into
// mesh.posClass at load time. When that map is present we CONSUME it (the fast
// path) instead of rehashing; otherwise (hand-built meshes) we weld here.
void weldCorners(const Mesh& mesh, ExtractContext& x) {
  const std::size_t nCorner = x.nCorner;
  x.corner2v.resize(nCorner);
  const bool usePosClass =
      mesh.posClass.size() == mesh.vertexCount() && mesh.posClassCount > 0;
  if (usePosClass) {
    // The reader assigned class ids in first-seen corner order with the same
    // weldKey, and we visit corners in that same order, so the representative
    // position (first corner of each class) and the per-class normal sum
    // (accumulated in corner order) are bit-for-bit identical to the weld below.
    x.nV = static_cast<std::size_t>(mesh.posClassCount);
    x.vpos.assign(x.nV, Vec3{0.0f, 0.0f, 0.0f});
    x.vacc.assign(x.nV, Vec3{0.0f, 0.0f, 0.0f});
    if (x.haveCol) {
      x.vAlphaSum.assign(x.nV, 0.0f);
      x.vAlphaCnt.assign(x.nV, 0);
    }
    std::vector<char> seen(x.nV, 0);
    for (std::size_t c = 0; c < nCorner; ++c) {
      const std::size_t v =
          static_cast<std::size_t>(mesh.posClass[mesh.cornerVertex(c)]);
      x.corner2v[c] = static_cast<int>(v);
      if (!seen[v]) {
        x.vpos[v] = cornerPos(mesh, c);
        seen[v] = 1;
      }
      if (x.haveNrm) x.vacc[v] = x.vacc[v] + cornerNrm(mesh, c);
      if (x.haveCol) {
        x.vAlphaSum[v] += cornerAlpha(mesh, c);
        x.vAlphaCnt[v]++;
      }
    }
  } else {
    std::unordered_map<WeldKey, int, WeldKeyHash> vmap;
    vmap.reserve(nCorner);
    for (std::size_t c = 0; c < nCorner; ++c) {
      const WeldKey k = weldKey(cornerPos(mesh, c));
      auto it = vmap.find(k);
      if (it == vmap.end()) {
        const int vi = static_cast<int>(x.vpos.size());
        vmap.emplace(k, vi);
        x.vpos.push_back(cornerPos(mesh, c));
        x.vacc.push_back(x.haveNrm ? cornerNrm(mesh, c)
                                   : Vec3{0.0f, 0.0f, 0.0f});
        if (x.haveCol) {
          x.vAlphaSum.push_back(cornerAlpha(mesh, c));
          x.vAlphaCnt.push_back(1);
        }
        x.corner2v[c] = vi;
      } else {
        x.corner2v[c] = it->second;
        if (x.haveNrm)
          x.vacc[static_cast<std::size_t>(it->second)] =
              x.vacc[static_cast<std::size_t>(it->second)] + cornerNrm(mesh, c);
        if (x.haveCol) {
          x.vAlphaSum[static_cast<std::size_t>(it->second)] +=
              cornerAlpha(mesh, c);
          x.vAlphaCnt[static_cast<std::size_t>(it->second)]++;
        }
      }
    }
    x.nV = x.vpos.size();
  }
}

// 2) Face welded indices, geometric/averaged/cluster normals, mean edge
//    length, and the welded edge adjacency (emap + faceNbr). Also publishes
//    the welded vertex table and per-vertex alpha into the result.
void buildFacesAndAdjacency(const Mesh& mesh, ExtractContext& x,
                            FeatureMesh& result) {
  const std::size_t nTri = x.nTri;
  const std::size_t nCorner = x.nCorner;
  const std::size_t nV = x.nV;

  x.fa.resize(nTri);
  x.fb.resize(nTri);
  x.fc.resize(nTri);
  x.fNg.resize(nTri);
  for (std::size_t f = 0; f < nTri; ++f) {
    x.fa[f] = x.corner2v[3 * f];
    x.fb[f] = x.corner2v[3 * f + 1];
    x.fc[f] = x.corner2v[3 * f + 2];
    x.fNg[f] = normalize(cross(x.vpos[static_cast<std::size_t>(x.fb[f])] - x.vpos[static_cast<std::size_t>(x.fa[f])],
                               x.vpos[static_cast<std::size_t>(x.fc[f])] - x.vpos[static_cast<std::size_t>(x.fa[f])]));
  }
  x.vN.assign(nV, Vec3{0.0f, 0.0f, 0.0f});
  for (std::size_t v = 0; v < nV; ++v) {
    const float l = length(x.vacc[v]);
    if (x.haveNrm && l > 1.0e-6f) x.vN[v] = x.vacc[v] * (1.0f / l);
  }
  for (std::size_t f = 0; f < nTri; ++f)
    for (int e = 0; e < 3; ++e) {
      const std::size_t v = static_cast<std::size_t>(e == 0   ? x.fa[f]
                                                     : e == 1 ? x.fb[f]
                                                              : x.fc[f]);
      if (length(x.vN[v]) < 0.5f) x.vN[v] = x.fNg[f];
    }

  result.vpos = x.vpos;

  // Per welded-vertex alpha (averaged corner opacity) for the edge_line2
  // transmissive endpoint fade. Left EMPTY when the mesh carries no per-vertex
  // color, so the edges stay uniformly opaque (FeatureSeg::alpha* keep their 1
  // default => opacity1 == -1).
  if (x.haveCol) {
    result.valpha.assign(nV, 1.0f);
    for (std::size_t v = 0; v < nV; ++v)
      if (x.vAlphaCnt[v] > 0)
        result.valpha[v] = x.vAlphaSum[v] / static_cast<float>(x.vAlphaCnt[v]);
  }

  // Mean triangle edge length, computed unconditionally (independent of which
  // natures are enabled) so the stroke pass can derive the obj-edges silhouette
  // camBias even when extracting with opt.silhouette off. This is the SAME
  // elsum/(3*nTri) the silhouette block computes locally (kept there to preserve
  // --obj-edges byte-identity); both produce the identical value.
  {
    double elsum = 0.0;
    for (std::size_t f = 0; f < nTri; ++f) {
      const std::size_t a = static_cast<std::size_t>(x.fa[f]);
      const std::size_t b = static_cast<std::size_t>(x.fb[f]);
      const std::size_t c = static_cast<std::size_t>(x.fc[f]);
      elsum += length(x.vpos[b] - x.vpos[a]) + length(x.vpos[c] - x.vpos[b]) +
               length(x.vpos[a] - x.vpos[c]);
    }
    result.meanEdge = nTri ? static_cast<float>(elsum / (3.0 * nTri)) : 0.0f;
  }

  // Edge adjacency (welded-position topology), SHARED by the hard-edge
  // silhouette and the crease/border pass.
  x.emap.reserve(nTri * 3);
  for (std::size_t f = 0; f < nTri; ++f) {
    const int v[3] = {x.fa[f], x.fb[f], x.fc[f]};
    for (int e = 0; e < 3; ++e) {
      EAdj& adj = x.emap[ekey(v[e], v[(e + 1) % 3])];
      if (adj.f1 < 0)
        adj.f1 = static_cast<int>(f);
      else if (adj.f2 < 0)
        adj.f2 = static_cast<int>(f);
    }
  }

  // Face -> up to 3 EDGE-adjacent neighbour faces (TRUE surface adjacency via the
  // welded edge map). A BFS over this follows the actual surface and does NOT jump
  // across a fold/gap the way a vertex 1-ring does: a fold's two sides
  // share at most a welded VERTEX, not a welded EDGE. So an N-ring exclude reaches
  // a geodesically-NEAR self-occluder (a ribbon's own surface a few faces across
  // its width) yet not a geodesically-FAR one (a tube's opposite wall / another
  // strand, many faces away) -- the discriminator the vertex 1-ring lacked.
  x.faceNbr.assign(nTri, std::array<int, 3>{-1, -1, -1});
  for (std::size_t f = 0; f < nTri; ++f) {
    const int v[3] = {x.fa[f], x.fb[f], x.fc[f]};
    for (int e = 0; e < 3; ++e) {
      auto it = x.emap.find(ekey(v[e], v[(e + 1) % 3]));
      if (it == x.emap.end()) continue;
      const EAdj& adj = it->second;
      x.faceNbr[f][static_cast<std::size_t>(e)] =
          (adj.f1 == static_cast<int>(f)) ? adj.f2 : adj.f1;
    }
  }

  // Per-CORNER shading normal that PRESERVES hard edges (smoothing clusters).
  x.cnrm.resize(nCorner);
  {
    std::vector<std::vector<int>> posCorners(nV);
    for (std::size_t c = 0; c < nCorner; ++c)
      posCorners[static_cast<std::size_t>(x.corner2v[c])].push_back(static_cast<int>(c));
    for (std::size_t v = 0; v < nV; ++v) {
      const std::vector<int>& cs = posCorners[v];
      std::vector<Vec3> rep, sum;
      std::vector<int> cl(cs.size());
      for (std::size_t k = 0; k < cs.size(); ++k) {
        const std::size_t c = static_cast<std::size_t>(cs[k]);
        Vec3 n = x.haveNrm ? cornerNrm(mesh, c) : x.fNg[c / 3];
        const float ln = length(n);
        n = ln > 1.0e-12f ? n * (1.0f / ln) : x.fNg[c / 3];
        int found = -1;
        for (std::size_t t = 0; t < rep.size(); ++t)
          if (dot(rep[t], n) >= x.hardCos) { found = static_cast<int>(t); break; }
        if (found < 0) {
          rep.push_back(n);
          sum.push_back(n);
          found = static_cast<int>(rep.size()) - 1;
        } else {
          sum[static_cast<std::size_t>(found)] = sum[static_cast<std::size_t>(found)] + n;
        }
        cl[k] = found;
      }
      std::vector<Vec3> avg(rep.size());
      for (std::size_t t = 0; t < rep.size(); ++t) {
        const float l = length(sum[t]);
        avg[t] = l > 1.0e-12f ? sum[t] * (1.0f / l) : rep[t];
      }
      for (std::size_t k = 0; k < cs.size(); ++k)
        x.cnrm[static_cast<std::size_t>(cs[k])] = avg[static_cast<std::size_t>(cl[k])];
    }
  }
}

// CONCAVE-fold test from the two adjacent faces' GEOMETRIC normals (the dihedral
// the faces make across edge a-b): true when the surface folds INWARD (a valley,
// angle < 180deg). The average face normal points TOWARD the two faces' opposite
// apex vertices for a valley and AWAY for a convex ridge, so dot(nAvg, apexMid -
// edgeMid) > 0 marks a concave edge. An edge with fewer than two faces (a border)
// has no dihedral and is never concave. Used by opt.rejectConcaveEdges to drop
// concave feature edges across ALL natures (NPR convention: ink only convex
// ridges / silhouettes). MUST use the geometric face normals here, by request.
bool isConcaveEdge(const ExtractContext& x, int f1, int f2, std::size_t a,
                   std::size_t b) {
  if (f1 < 0 || f2 < 0) return false;
  const std::size_t g1 = static_cast<std::size_t>(f1);
  const std::size_t g2 = static_cast<std::size_t>(f2);
  auto apex = [&](std::size_t f) -> Vec3 {
    const std::size_t xx = static_cast<std::size_t>(x.fa[f]);
    const std::size_t yy = static_cast<std::size_t>(x.fb[f]);
    const std::size_t zz = static_cast<std::size_t>(x.fc[f]);
    if (xx != a && xx != b) return x.vpos[xx];
    if (yy != a && yy != b) return x.vpos[yy];
    return x.vpos[zz];
  };
  const Vec3 nAvg = normalize(x.fNg[g1] + x.fNg[g2]);
  const Vec3 edgeMid = (x.vpos[a] + x.vpos[b]) * 0.5f;
  const Vec3 apexMid = (apex(g1) + apex(g2)) * 0.5f;
  return dot(nAvg, apexMid - edgeMid) > 0.0f;
}

// Synthetic chain node id of the smooth-contour crossing on welded edge
// (wa,wb); created on first use, shared by both incident faces.
int crossingNodeId(ExtractContext& x, int wa, int wb) {
  const std::uint64_t key = ekey(wa, wb);
  auto it = x.crossNode.find(key);
  if (it != x.crossNode.end()) return it->second;
  const int id = x.nextNode++;
  x.crossNode.emplace(key, id);
  return id;
}

// Build segment's QI self-face exclude set: the LITERAL incident faces
// {f0,f1} (f1<0 border = no-op), optionally grown by kSelfRings edge-adjacency
// rings. The edge's own surface is then suppressed by these ids PLUS the
// geometric COINCIDENT-PLANE skip in the QI ray cast
// (EmbreeRenderer::occluded coplanarEps -- Freestyle GeomUtils::COINCIDENT): a
// hit whose plane passes through the silhouette point is the point's OWN surface
// and is not counted. That geometric test REPLACES the old topological vertex-
// 1-ring, which could not distinguish a continuous self-surface (must skip, to
// keep the outer outline solid) from a real front occluder that merely shares a
// weld vertex on a tightly self-folded ribbon (must count, or the back fold
// leaks over the front). The plane test handles both: the self-surface is
// coplanar with the point (skip); the front occluder is a fold-gap away (count).
std::vector<int> buildExclude(const ExtractContext& x, int f0, int f1) {
  std::vector<int> ex;
  ex.reserve(x.kSelfRings > 0 ? 16 : 4);
  auto addF = [&](int f) {
    if (f < 0) return;
    for (int g : ex)
      if (g == f) return;
    ex.push_back(f);
  };
  addF(f0);  // the edge's primary feature face
  addF(f1);  // the second fold/adjacent face (border f1<0 = no-op)
  // Grow kSelfRings edge-adjacency rings over the TRUE surface (faceNbr) so the
  // geodesically-near self-surface is skipped without jumping a fold (see above).
  std::size_t frontier = 0;
  for (int ring = 0; ring < x.kSelfRings; ++ring) {
    const std::size_t end = ex.size();
    for (std::size_t i = frontier; i < end; ++i)
      for (int nb : x.faceNbr[static_cast<std::size_t>(ex[i])]) addF(nb);
    frontier = end;
    if (ex.size() == end) break;  // ring did not grow: whole component covered
  }
  return ex;
}

// Emit a feature segment, carrying its incident mesh-triangle ids (f0,f1; -1
// when absent) for Freestyle self-face exclusion, plus the self-face exclude
// set used by the stroke pass's visibility ray-cast.
void emitSeg(ExtractContext& x, FeatureMesh& result, int n0, const Vec3& A,
             int n1, const Vec3& B, EdgeNature nat, std::uint16_t grp, int f0,
             int f1, const Vec3& nrm = Vec3{0.0f, 0.0f, 0.0f}) {
  FeatureSeg s{n0, n1, A, B, nat, grp, f0, f1, {}};
  s.excludeFaces = buildExclude(x, f0, f1);
  s.nrm = nrm;  // nonzero only for the smooth n.v==0 contour (cusp detection)
  result.segs.push_back(std::move(s));
}

// Border/crease emit helper: lift endpoints outward along the vertex normal by
// opt.raise (matching emitMeshEdges::push) and tag with welded node ids plus
// the two incident faces (f1 < 0 for a single-face border edge).
void pushEdge(ExtractContext& x, FeatureMesh& result, float raise,
              std::size_t a, std::size_t b, EdgeNature nat, std::uint16_t grp,
              int f0, int f1) {
  emitSeg(x, result, static_cast<int>(a), x.vpos[a] + x.vN[a] * raise,
          static_cast<int>(b), x.vpos[b] + x.vN[b] * raise, nat, grp, f0, f1);
}

// 3) SILHOUETTE = smooth n.v==0 contour (3a) + hard-edge straddle (3b).
void detectSilhouettes(const Mesh& mesh, const Camera& cam,
                       const ExtractParams& opt, ExtractContext& x,
                       FeatureMesh& result) {
  const std::size_t nTri = x.nTri;
  const float w = opt.width > 0.0f ? opt.width : 0.0f;
  const float silOff = opt.raise + w;
  double elsum = 0.0;
  for (std::size_t f = 0; f < nTri; ++f) {
    const std::size_t a = static_cast<std::size_t>(x.fa[f]);
    const std::size_t b = static_cast<std::size_t>(x.fb[f]);
    const std::size_t c = static_cast<std::size_t>(x.fc[f]);
    elsum += length(x.vpos[b] - x.vpos[a]) + length(x.vpos[c] - x.vpos[b]) +
             length(x.vpos[a] - x.vpos[c]);
  }
  const float meanEdge = nTri ? static_cast<float>(elsum / (3.0 * nTri)) : 0.0f;
  // Eye-ward bias that lifts a silhouette off its grazing tangent shell. Kept for
  // --obj-edges (3D cylinder visibility); zeroed for the stroke/--edges pass
  // (opt.silhouetteCamBias==false) so its ray-cast QI starts on the TRUE surface
  // point (Freestyle center3d), not a point pulled toward the eye that floats in
  // front of its own occluder and wrongly votes visible.
  const float camBias =
      opt.silhouetteCamBias ? std::fmax(0.5f * w, 0.15f * meanEdge) : 0.0f;

  // Silhouette method selector (opt.geomSilhouette): false (default) = SMOOTH
  // contour 3a (interpolated n.v==0, sub-triangle crossings; matches the smooth-
  // shaded outline) plus 3b only on hard edges. true = GEOMETRIC per-edge
  // silhouette: disable 3a and apply the 3b face-normal straddle to ALL edges
  // (hard-edge gate removed), so the silhouette follows mesh edges at mesh
  // resolution. Each has trade-offs (3a smooth but sparse at grazing folds; 3b
  // mesh-dense but faceted and broken under grazing QI), hence the toggle.
  const bool geomSilOnly = opt.geomSilhouette;

  // 3a) SMOOTH silhouette: per face, connect the two n.v==0 zero-crossings.
  for (std::size_t f = 0; !geomSilOnly && f < nTri; ++f) {
    const int cc[3] = {static_cast<int>(3 * f), static_cast<int>(3 * f + 1),
                       static_cast<int>(3 * f + 2)};
    const int wv[3] = {x.corner2v[static_cast<std::size_t>(cc[0])],
                       x.corner2v[static_cast<std::size_t>(cc[1])],
                       x.corner2v[static_cast<std::size_t>(cc[2])]};
    Vec3 P[3], NN[3];
    float d[3];
    for (int k = 0; k < 3; ++k) {
      P[k] = x.vpos[static_cast<std::size_t>(wv[k])];
      NN[k] = x.cnrm[static_cast<std::size_t>(cc[k])];
      d[k] = dot(NN[k], viewerDirAt(P[k], cam));
    }
    Vec3 cp[2], cn[2];
    int cid[2];
    // The other mesh face across the welded edge each crossing lands on (-1 if
    // the crossing is on a vertex or a border edge): the contour segment lives
    // inside face f, but on a smooth surface the adjacent face shares the same
    // grazing tangent, so it is an incident self-face too. Carried so the
    // visibility ray-cast excludes it (Freestyle adjacent-face exclusion).
    int cnb[2] = {-1, -1};
    int nc = 0;
    // A crossing on triangle edge (i,j) keys its synthetic node on the welded
    // edge (wv[i], wv[j]); a crossing landing on a vertex uses its welded id.
    auto addCross = [&](int i, int j) {
      const float t = d[i] / (d[i] - d[j]);
      cp[nc] = P[i] + (P[j] - P[i]) * t;
      cn[nc] = normalize(NN[i] + (NN[j] - NN[i]) * t);
      cid[nc] = crossingNodeId(x, wv[i], wv[j]);
      const auto it = x.emap.find(ekey(wv[i], wv[j]));
      if (it != x.emap.end()) {
        const EAdj& a = it->second;
        cnb[nc] = (a.f1 == static_cast<int>(f)) ? a.f2 : a.f1;
      }
      ++nc;
    };
    auto addVert = [&](int i) {
      cp[nc] = P[i];
      cn[nc] = NN[i];
      cid[nc] = wv[i];
      ++nc;
    };
    int nullIdx[3] = {-1, -1, -1};
    int nnull = 0;
    for (int k = 0; k < 3; ++k)
      if (d[k] == 0.0f) nullIdx[nnull++] = k;
    if (nnull == 0) {
      for (int e = 0; e < 3 && nc < 2; ++e)
        if (d[e] * d[(e + 1) % 3] < 0.0f) addCross(e, (e + 1) % 3);
    } else if (nnull == 1) {
      const int o0 = (nullIdx[0] + 1) % 3, o1 = (nullIdx[0] + 2) % 3;
      if (d[o0] * d[o1] < 0.0f) {
        addVert(nullIdx[0]);
        addCross(o0, o1);
      }
    } else if (nnull == 2) {
      addVert(nullIdx[0]);
      addVert(nullIdx[1]);
    }
    if (nc == 2) {
      const Vec3 v0 = viewerDirAt(cp[0], cam) * camBias;
      const Vec3 v1 = viewerDirAt(cp[1], cam) * camBias;
      // Incident faces: the contour segment lives inside face f (face0); the
      // two faces across the welded edges its endpoints crossed are adjacent
      // self-faces on the smooth surface (face1 = the first available one).
      const int adj = cnb[0] >= 0 ? cnb[0] : cnb[1];
      // Segment surface normal for cusp detection: the average of the two
      // crossing-point interpolated normals (Freestyle FEdgeSmooth::normal()).
      const Vec3 segN = normalize(cn[0] + cn[1]);
      emitSeg(x, result, cid[0], cp[0] + cn[0] * silOff + v0, cid[1],
              cp[1] + cn[1] * silOff + v1, EdgeNature::Silhouette,
              mesh.groupForTri(f), static_cast<int>(f), adj, segN);
    }
  }

  // 3b) HARD-EDGE silhouette (face-normal straddle).
  for (const auto& kv : x.emap) {
    const EAdj& adj = kv.second;
    if (adj.f2 < 0) continue;  // border edge: handled in crease/border pass
    const std::size_t f1 = static_cast<std::size_t>(adj.f1);
    const std::size_t f2 = static_cast<std::size_t>(adj.f2);
    if (!geomSilOnly && dot(x.fNg[f1], x.fNg[f2]) >= x.hardCos)
      continue;  // smooth edge: 3a covers it (unless geom-sil diagnostic)
    const std::size_t a = static_cast<std::size_t>(kv.first & 0xffffffffu);
    const std::size_t b = static_cast<std::size_t>(kv.first >> 32);
    const Vec3 da = viewerDirAt(x.vpos[a], cam);
    const Vec3 db = viewerDirAt(x.vpos[b], cam);
    const bool straddleA = dot(da, x.fNg[f1]) * dot(da, x.fNg[f2]) < 0.0f;
    const bool straddleB = dot(db, x.fNg[f1]) * dot(db, x.fNg[f2]) < 0.0f;
    if (!straddleA && !straddleB) continue;
    // Drop a CONCAVE hard edge (valley fold) from the silhouette candidates.
    if (opt.rejectConcaveEdges && isConcaveEdge(x, adj.f1, adj.f2, a, b))
      continue;
    const Vec3 outw = normalize(x.fNg[f1] + x.fNg[f2]);
    emitSeg(x, result, static_cast<int>(a),
            x.vpos[a] + outw * silOff + da * camBias, static_cast<int>(b),
            x.vpos[b] + outw * silOff + db * camBias, EdgeNature::Silhouette,
            mesh.groupForTri(adj.f1), adj.f1, adj.f2);
  }
}

// 4) CREASE (dihedral) + BORDER (single-face) on mesh edges.
void detectCreasesAndBorders(const Mesh& mesh, const ExtractParams& opt,
                             ExtractContext& x, FeatureMesh& result) {
  const std::size_t nV = x.nV;
  const float creaseCos = std::cos(opt.creaseAngleDeg * kPi / 180.0f);
  const bool smoothVeto = opt.meshCreaseSmoothVetoDeg > 0.0f;
  const float smoothCos = std::cos(opt.meshCreaseSmoothVetoDeg * kPi / 180.0f);
  const bool borderVeto = opt.meshBorderCoplanarVetoDeg > 0.0f;
  const float borderColinCos =
      std::cos(opt.meshBorderCoplanarVetoDeg * kPi / 180.0f);

  // Per-corner SHADING normal of face f at welded vertex v -- the RAW vertex
  // normal the surface actually interpolates (mesh.normals), NOT the hard-edge
  // cluster normal. The smooth-shaded-across-edge crease veto compares these
  // across the edge: if face f1 and face f2 carry the same interpolated normal
  // at the two shared vertices, the surface is smooth-shaded across the edge
  // (a tessellation facet on a curved-but-smooth tube -- a cap ring or a tight
  // helix turn), not a real fold. Using the raw normals (not the cluster ones)
  // is what makes the veto robust at TIGHT curvature: there the facet dihedral
  // can exceed the hard-edge cluster split, yet the provided vertex normals
  // still agree, so the false tick is correctly suppressed. No provided normals
  // => fall back to the geometric face normal, so a genuine flat-shaded fold
  // (disagreeing face normals) still survives the veto.
  auto cornerNrmOf = [&](int f, std::size_t v) -> Vec3 {
    const int c0 = 3 * f;
    int c = c0;
    if (static_cast<std::size_t>(x.corner2v[static_cast<std::size_t>(c0 + 1)]) == v)
      c = c0 + 1;
    else if (static_cast<std::size_t>(x.corner2v[static_cast<std::size_t>(c0 + 2)]) == v)
      c = c0 + 2;
    if (!x.haveNrm) return x.fNg[static_cast<std::size_t>(f)];
    const Vec3 n = cornerNrm(mesh, static_cast<std::size_t>(c));
    const float l = length(n);
    return l > 1.0e-12f ? n * (1.0f / l) : x.fNg[static_cast<std::size_t>(f)];
  };

  std::unordered_map<int, std::vector<int>> borderAdj;
  if (opt.border && borderVeto) {
    borderAdj.reserve(nV);
    for (const auto& kv : x.emap) {
      if (kv.second.f2 >= 0) continue;
      const int a = static_cast<int>(kv.first & 0xffffffffu);
      const int b = static_cast<int>(kv.first >> 32);
      borderAdj[a].push_back(b);
      borderAdj[b].push_back(a);
    }
  }
  auto borderContinues = [&](std::size_t p, std::size_t other) -> bool {
    const auto it = borderAdj.find(static_cast<int>(p));
    if (it == borderAdj.end()) return false;
    const Vec3 in = normalize(x.vpos[p] - x.vpos[other]);
    for (const int q : it->second) {
      if (static_cast<std::size_t>(q) == other) continue;
      const Vec3 outDir = normalize(x.vpos[static_cast<std::size_t>(q)] - x.vpos[p]);
      if (dot(in, outDir) >= borderColinCos) return true;
    }
    return false;
  };

  struct CreaseCand {
    std::size_t a, b;
    std::uint16_t grp;
    int f0, f1;  // the two incident faces (crease is always interior)
  };
  std::vector<CreaseCand> creaseCand;
  std::vector<int> creaseDeg;
  const int maxDeg = opt.meshCreaseMaxDegree;
  if (maxDeg > 0) creaseDeg.assign(nV, 0);

  for (const auto& kv : x.emap) {
    const std::size_t a = static_cast<std::size_t>(kv.first & 0xffffffffu);
    const std::size_t b = static_cast<std::size_t>(kv.first >> 32);
    const EAdj& adj = kv.second;

    if (adj.f2 < 0) {
      if (!opt.border) continue;
      if (borderVeto && borderContinues(a, b) && borderContinues(b, a)) continue;
      pushEdge(x, result, opt.raise, a, b, EdgeNature::Border,
               mesh.groupForTri(static_cast<std::size_t>(adj.f1)), adj.f1, -1);
      continue;
    }

    if (!opt.crease) continue;
    const std::size_t f1 = static_cast<std::size_t>(adj.f1);
    const std::size_t f2 = static_cast<std::size_t>(adj.f2);
    if (dot(x.fNg[f1], x.fNg[f2]) >= creaseCos) continue;

    // Smooth-facet crease veto (Freestyle fidelity). Suppress a "crease" where
    // the two faces are SMOOTH-SHADED across the edge: their interpolated
    // (cluster) vertex normals agree, so the edge is a tessellation facet on a
    // curved-but-smooth surface (a tube cap ring, a tight helix turn), not a
    // real fold. A genuine sharp fold (e.g. a beta-sheet box edge) splits the
    // hard-edge clusters, so its corner normals DISAGREE across the edge and it
    // survives. Comparing the two faces' own corner normals (not the global
    // welded average vs the geometric facet normals) is robust on tight
    // curvature, where the welded average can deviate from a facet by more than
    // the veto angle and let a false tick through.
    if (smoothVeto) {
      const Vec3 nA1 = cornerNrmOf(adj.f1, a), nA2 = cornerNrmOf(adj.f2, a);
      const Vec3 nB1 = cornerNrmOf(adj.f1, b), nB2 = cornerNrmOf(adj.f2, b);
      const float agree = std::fmin(dot(nA1, nA2), dot(nB1, nB2));
      if (agree >= smoothCos) continue;  // smooth-shaded across edge: no crease
    }

    // Drop a CONCAVE crease (valley fold). meshCreaseConvexOnly is the legacy
    // crease-specific gate; rejectConcaveEdges is the unified all-nature gate.
    if ((opt.meshCreaseConvexOnly || opt.rejectConcaveEdges) &&
        isConcaveEdge(x, adj.f1, adj.f2, a, b))
      continue;

    const std::uint16_t grp = mesh.groupForTri(static_cast<std::size_t>(adj.f1));
    if (maxDeg > 0) {
      creaseDeg[a]++;
      creaseDeg[b]++;
      creaseCand.push_back({a, b, grp, adj.f1, adj.f2});
    } else {
      pushEdge(x, result, opt.raise, a, b, EdgeNature::Crease, grp, adj.f1,
               adj.f2);
    }
  }
  for (const CreaseCand& c : creaseCand)
    if (creaseDeg[c.a] <= maxDeg && creaseDeg[c.b] <= maxDeg)
      pushEdge(x, result, opt.raise, c.a, c.b, EdgeNature::Crease, c.grp, c.f0,
               c.f1);
}

}  // namespace

FeatureMesh extractMeshFeatureEdges(const Mesh& mesh, const Camera& cam,
                                    const ExtractParams& opt) {
  FeatureMesh result;
  ExtractContext x;
  x.nCorner = mesh.cornerCount();
  x.nTri = x.nCorner / 3;
  if (x.nTri == 0) return result;
  x.haveNrm = mesh.normals.size() == mesh.vertexCount();
  x.haveCol = mesh.colors.size() == mesh.vertexCount();
  x.hardCos = std::cos(opt.meshHardEdgeDeg * kPi / 180.0f);
  // Self-exclude geodesic radius (edge-adjacency rings beyond the incident
  // faces); 0 => the byte-identical default ({f0,f1} only). See
  // ExtractParams::selfExcludeRings.
  x.kSelfRings = opt.selfExcludeRings > 0 ? opt.selfExcludeRings : 0;

  weldCorners(mesh, x);
  x.nextNode = static_cast<int>(x.nV);
  buildFacesAndAdjacency(mesh, x, result);
  if (opt.silhouette) detectSilhouettes(mesh, cam, opt, x, result);
  if (opt.crease || opt.border) detectCreasesAndBorders(mesh, opt, x, result);

  // Tag each segment's endpoint opacity from the welded-vertex alpha. Welded-id
  // endpoints (hard-edge / crease / border, v < nV) get their vertex alpha;
  // synthetic smooth-contour crossings (v >= nV, no single vertex) keep the
  // opaque default. No-op when the mesh has no per-vertex color (valpha empty).
  if (!result.valpha.empty()) {
    const int nVi = static_cast<int>(x.nV);
    for (FeatureSeg& seg : result.segs) {
      if (seg.v0 >= 0 && seg.v0 < nVi)
        seg.alpha0 = result.valpha[static_cast<std::size_t>(seg.v0)];
      if (seg.v1 >= 0 && seg.v1 < nVi)
        seg.alpha1 = result.valpha[static_cast<std::size_t>(seg.v1)];
    }
  }

  result.nodeCount = x.nextNode;
  return result;
}

}  // namespace umbreon
