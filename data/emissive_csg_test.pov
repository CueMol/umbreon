/*
  CSG-emitter GI test scene (pt2 CSG emissive NEE milestone renders).
  A white back wall with an emissive orange CSG sphere and an emissive blue
  capped cylinder floating in front of it: under --integrator pt2 with
  --pt2-emissive-nee on both primitives are sampled as light sources (low
  noise at draft spp); with the flag off they light the wall through BSDF
  sampling only (same converged mean, far noisier). Header structure follows
  data/emissive_test.pov (the bench reader's dialect).
 */
...
// Dim direct lighting (vs the emissive_test defaults) so the emitters'
// indirect halo carries the image instead of a near-saturated wall.
#ifndef (_light_inten)
  #declare _light_inten=0.45;
...
#declare _scene = #include "emissive_csg_test.inc"
