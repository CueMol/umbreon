/*
  Minimal emissive-GI test scene (pt2 milestone renders + tests).
  A white back wall with a small emissive quad floating in front of it:
  under --integrator pt2 (emissive on) the wall shows an orange indirect
  halo around the quad; pt1 shows only the quad's own self-illumination.
  Header structure follows data/test1.pov (the bench reader's dialect).
 */
...
#declare _scene = #include "emissive_test.inc"
