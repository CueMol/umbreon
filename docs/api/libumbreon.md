...
| 透過（front-to-back over ＋ group alpha 多重パス blend） | あり | `RenderOptions::transparency`, `Scene::groupBlend` |
...
**OIDN はリンク依存ではありません**: OIDN デノイザ（`RenderOptions::denoiser = 2`）は
**プロセス外**で動きます。`UMBREON_WITH_OIDN=ON` のビルドは、ライブラリに IPC クライアント
（Boost header-only、ビルド時のみの依存で consumer へは伝播しない）を組み込み、OIDN を
リンクする唯一のバイナリ **`umbreon_oidn_worker`**（`bin/` にインストールされる）を併せて
ビルドします。実行時にワーカーが見つからない／失敗した場合は内蔵の a-trous デノイザへ
フォールバックします（探索順は `RenderOptions::oidnWorkerPath` のコメントを参照:
明示パス → 環境変数 `UMBREON_OIDN_WORKER` → ホスト実行ファイル隣 → PATH）。
...
#### 非同期レンダリングと進捗（数分かかる描画向け）

`render()` はブロッキングだが、進捗の取得とキャンセルのために2つの追加インタフェイスがある
（`<umbreon/render/render_progress.hpp>`）。数分かかる描画で「どこまで進んだか」を表示したり、
視点変更・ダイアログ閉じで実行中の描画を中断したりできる。

```cpp
// (1) 低レベル: 進捗・キャンセルチャネルを渡す同期版。呼び出し側が自前スレッドで回す。
FrameResult render(const Scene&, const RenderOptions&, RenderProgress&);

// (2) 高レベル: ライブラリがワーカースレッドを所有する非同期ハンドル。
RenderTask renderAsync(Scene scene, RenderOptions opt);  // 即 return（scene/opt は値渡し）
```

`RenderProgress`（**全フィールド atomic・ロックフリー**。任意スレッドから安全に読める）:
...
`RenderTask`（move-only。デストラクタが cancel + join するためスレッドリークしない）:

- `progress()` / `phase()` / `done()` / `cancel()` / `wait_for(ms)` / `wait()` /
  `get()`（完了まで join し `FrameResult` を返す。描画中に投げられた例外はここで再送出）。

進捗フックは atomic 加算のみでピクセル値に一切影響しないため、`renderAsync().get()` は
2引数 `render()` と**バイト同一**の結果を返す（決定論保証を維持）。CLI（`umbreon_cli`）は
このAPIでテキスト進捗バーを描画する（TTY 時のみ、stderr）。使用例は
`tests/test_render_async.cpp` と `src/bench/main.cpp` を参照。

#### OIDN デノイザ制御（プロセス外 worker）

OIDN デノイザ（`RenderOptions::denoiser == 2`、および pt1 の `RenderOptions::pt1Denoise`）は
**別プロセス** `umbreon_oidn_worker` で実行される（libumbreon 自体は OIDN をリンクしない）。
worker はライブラリが遅延起動して再利用する。次の2関数でその常駐 worker を明示制御できる
（どちらもスレッドセーフ、worker 非搭載ビルドでも安全に呼べる）。

```cpp
// worker を解決・起動しハンドシェイクまで行う（成功時 worker は warm のまま）。
// workerPath は RenderOptions::oidnWorkerPath と同義: 非空なら唯一の候補、
// 空なら env UMBREON_OIDN_WORKER → 実行ファイル隣接 → PATH の順で探索。
// 起動できなければ false（worker 非搭載ビルドでは常に false）。
bool oidnDenoiserAvailable(const std::string& workerPath = std::string());

// 常駐 worker を graceful に停止（stdin close → 最大2秒待機 → 必要なら kill）し
// 共有メモリを解放。次の描画/probe で再起動する（disabled ラッチも解除）。
void shutdownOidnDenoiser();
```

- **用途**: UI の事前判定（「GI denoise 利用可か」を `oidnDenoiserAvailable()` で表示）、初回描画前の
  worker ウォームアップ、レンダリングセッション間のメモリ解放（`shutdownOidnDenoiser()`）。
- worker が見つからない／失敗した場合、描画は内蔵 a-trous デノイザへ**自動フォールバック**する
  （stderr 警告）。実際に走ったデノイザは `FrameResult::denoiserUsed` / `pt1DenoiserUsed`（§4.7）で確認できる。
- **配備**は §7 と `docs/deploy/cuemol-oidn-worker.md`（CueMol 統合ガイド）を参照。パッケージ済み
  アプリでは worker は実行ファイル隣接**ではない**場所に置かれるため、`oidnWorkerPath` の明示指定が
  一次機構になる。
...
| `denoiserUsed` | `int` | 実際に走った**最終カラー**デノイザ（0=None, 1=AtrousBilateral, 2=OIDN）。2 は OIDN worker が実処理したときのみ。1 は明示 a-trous **および全 OIDN フォールバック**（worker 不在/死亡/エラー、非搭載ビルド）を含む |
| `pt1DenoiserUsed` | `int` | pt1 の間接照度(E)デノイズ（`pt1Denoise`）について同上（0=未実行）。**CueMol の GI 経路（`giDenoise`）が見るべきはこちら**。group-alpha 多重パスは最終(carrier)パスの値 |
...
- **OIDN デノイズを使う場合**は `umbreon_oidn_worker` を配備する。worker は初回デノイズ時に
  1 度だけ起動され、以後のレンダで再利用される（OIDN デバイス初期化コストは初回のみ）。
  ホスト終了時（クラッシュ時含む）は stdin のパイプ閉鎖で自動終了する。
  - **レガシー uxp_gui**: worker を実行ファイル隣接（GreD）に置けば既定探索で自動発見（ゼロコンフィグ）。
  - **CueMol3 (Electron/tritium)**: パッケージ済みアプリでは worker は `Resources/cuemol2/bin/`
    にあり実行ファイル隣接**ではない**ため、JS 側で解決したパスを `RenderOptions::oidnWorkerPath`
    に明示指定する（blendpng と同じ配線）。
  - 詳細なファイルレベル統合手順は **`docs/deploy/cuemol-oidn-worker.md`**（CueMol 統合ガイド）を参照。
...
- `render()` はフレームごとに Embree デバイス＋BVH を再構築する。対話的に多数フレームを
  ...
  （現状は静止画・バッチ用途を想定）。数分かかる単発描画の進捗表示・中断は
  `renderAsync` / `RenderProgress`（§4.1）で対応済み。
- 進捗の途中経過**画像**プレビュー（描画中のフレームバッファのスナップショット）は未対応。
  進捗は割合・フェーズのみ。将来拡張候補。
