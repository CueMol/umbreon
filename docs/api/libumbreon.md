...
GGX principled subset を評価する。UI 側は「金属的 / プラスチック的 / 陶器的」等のプリセットを
本フィールドへ写像する想定。
...
| 値 | 用途 |
|---|---|
| `Pov`（既定） | POV finish モデル。**API としては非推奨** — umbreon_cli が `.pov` から POV-Ray 参照レンダーを bit-exact に再現するためのテスト経路であって、API 呼び出し側が選ぶルックではない。既定値はソース互換のため据置 |
| `Principled` | 物理材。本節の `Material::pbr` を読む |
| `Toon` | NPR 用の明示タグ。Pov と**同じ**非物理ローブで shading される（全ディスパッチが `model == Principled ? principled : pov` なので Toon は POV 側へ落ちる = **ピクセルは 1 つも動かない**）。効くのは `toonLike()` が値ヒューリスティックを介さず常に true を返す点 — フィールド値が物理的に見えても GI 免除と principled 変換除外が確実に効く |

新規の API シーンは `Principled`（物理材）か `Toon`（NPR）を明示的に指定する。`.pov` シーンは
NPR 意図を綴る手段がないため、Pov 側は `toonLike()` の値ヒューリスティックで拾う（bench の再現性用）。
...
| `pbr.anisotropy` | 0.0 | 異方性（Disney aspect 写像）。**sphere/cylinder プリミティブのみ有効**（sphere は **world +Y(縦軸) pole**、cylinder は軸フレーム。mesh は per-vertex tangent 未対応のため等方） |
...
| `pt1DenoiserUsed` | `int` | pt1 の間接照度(E)デノイズ（`pt1Denoise`）について同上（0=未実行）。**CueMol の GI 経路（`giDenoise`）が見るべきはこちら** |
...
- **スタイリング（方式A）**: `EdgeClass`（Silhouette / Disconnected / Object / Material / Crease）
  ...
  線を引かない）。外形線は背後に何があっても途切れない: 背景との境界に加え、Outline
  セクションが手前側となる他セクションとの境界もシルエット（`sil` スロット）として
  描かれるため、背後にメッシュ等の別セクションがあっても外形線は完全に描画される。
  Outline セクションが奥側の場合は従来通り手前セクションの Object 境界線が適用され、
  隠線は描かれない。モードの異なるセクションは 1 フレーム内で共存できる。
- **接触輪郭線（方式A）**: 深度が連続する**セクション間**の接触・交差境界（stick が別
  セクションの ribbon に刺さる交差輪郭など）は既定では描かれないが、
  `strokeEdges.contact = true`（CLI: `--stroke-contact on`）で描画できる（Full / Outline
  両モードに適用。同一セクション内の接触は常に描かない）。接触部では「手前側」が数値
...
- **OIDN メモリ**: OIDN デノイザ（`denoiser=2` / `pt1Denoise`）は `oidnMaxMemoryMB`（既定 1024）
  でスクラッチを内蔵タイル分割し、**単一確保サイズを解像度非依存に約 1GiB 以下**へ抑える。
  Electron の PartitionAlloc は単一確保 ~2GiB 超で abort（SIGTRAP）するため必須で、既定
  cap なしでは 13MP で単一 2.5GiB を確保して落ちる（実測）。`< 0` で OIDN 既定（cap なし）。
  値を変えるとタイル境界が変わり大画像では出力ビットが変わる（小画像は不発動）。
- **例外安全**: 失敗時は `std::runtime_error`。CueMol 側で try/catch すること。OIDN の
  ランタイム失敗時は例外ではなく内蔵 a-trous へフォールバックし、`FrameResult::denoiserUsed`
  に実績（1）が残る。
...
- **OIDN デノイズ**: `oidnMaxMemoryMB`（既定 1024）が単一確保を抑えるので、Electron の
  PartitionAlloc クラッシュ回避のために CueMol 側が持つ `OIDN_MAX_RENDER_AREA` 相当の
  解像度キャップは撤去できる（Electron 実機で大判レンダの動作確認後）。フォールバック発生は
  `FrameResult::pt1DenoiserUsed`（GI 経路）で検知できる。
