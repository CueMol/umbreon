...
> **前提の実装状況(2026-06)**: ao-quality.md は全フェーズ実装済み(`feat/ao-quality`)。`computeAOQuality`→`AOResult{contact,shape,openness,bent,avgHitDist}`、`aoWriteAov` ゲートの first-hit/G-buffer AOV(`albedo`/`normal`/`contactAo`/`shapeAo`/`bentNormal`/`avgHitDist`)主経路出力、bent 由来 2 色 sky/ground 方向性 ambient は利用可能。本プランは `position` AOV の追加と `IrradianceRecord`/cache 本体/denoiser から着手できる。
