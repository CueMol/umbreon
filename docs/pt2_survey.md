...
| 順 | 内容 | 目標 | 根拠・条件 |
|---|---|---|---|
| **C** | **多バウンス先行実験**(半日): ao_test1_hi / densurf で traced bounces=2/4/8 の収束差と時間を実測し、pt2.2 (screen-space 再利用) の go/no-go を決める | ① | pt2.2 は m2 と同じ構造では死なない(収束済み E バッファ参照 = 圧縮なし)が、「リアルタイム由来手法の前提崩れ」の前例を踏まえ実装前に価値を確認 |
| **B'** | **分散適応 spp**(§2-C): 凹部(高分散域)のみ追加サンプル。Cycles/appleseed 型、adaptive-AA と同じ 2-pass 決定論 | ② | ReSTIR に代わる「同コストで品質」の本命。確実・小規模・A' と直交 |
| **A'** | **full PT 化**: (1) glossy/specular BSDF の経路統合(旧 pt2.3 の拡張、フェイク reflection 置換)→ (2) area light + MIS → (3) emissive NEE(§2-F 第 2 段) | ③ + 厳密さ | distant/area/emissive × diffuse/glossy の unbiased PT に到達し、分子シーンで caustic 以外の経路空間を埋める。確立された手法(pbrt-v4 が教材)で手法リスクが低い。direct pass(POV 風解析シェーディング)との整合設計が肝 |
